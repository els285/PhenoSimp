...
from phenosimp.delphes.truth_tools import parse_tops_and_Ws, parse_decays, parse_meta, parse_reco, do_matching, examine_matched_indices
...
def main(inname,outname):
    ...
    assert ak.count_nonzero(ak.count(truth_dict["top_id"],axis=1) !=2 ) == 0, "There are events with other than four top quarks"
    assert ak.count_nonzero(ak.count(truth_dict["W_id"],axis=1)   !=2 ) == 0, "There are events with other than four on-shell W bosons"

    print("Parsing decay information")
    truth_dict = parse_decays(tree,truth_dict)
    assert ak.count_nonzero(ak.count(truth_dict["W_decay_id"],axis=1) !=4 ) == 0, "There are events with other than four bottom quarks"
    ...
    reco_dict = parse_reco(tree,reco_dict,do_substructure=True)
    ...
    reco_dict = examine_matched_indices(list_of_matched_indices,reco_dict,6)
