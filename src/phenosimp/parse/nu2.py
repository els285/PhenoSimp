...
def parse_jets(reco):
    ...
    max_jets = ak.max(ak.num(reco["jet_pt"].array()))
    jet_data = np.zeros((len(reco["jet_pt"].array()), max_jets), dtype=jet_dt)
    ...
    jet_data["is_tagged"]   = pad_variable(reco["jet_btag"].array() , max_jets)
    
    jet_data = jet_data.astype([('pt', '<f4'), ('eta', '<f4'), ('phi', '<f4'), ('energy', '<f4'), ('is_tagged', '<?')])

    return jet_data

def parse_leptons(reco):
    ...
    lepton_pTs = ak.concatenate([reco["el_pt"].array(),reco["mu_pt"].array()],axis=1)
    sorted_indices =  ak.argsort(lepton_pTs,ascending=False)
    
    lepton_vectors = lepton_vectors[sorted_indices]
    lepton_charge  = lepton_charge[sorted_indices]
    lepton_type    = lepton_type[sorted_indices]
    ...
    # Leptons
    lepton_data = np.zeros((len(lepton_pTs), Nleptons), dtype=leptons_dt)
...
def parse_neutrinos(truth):
    
    """
    Takes the truth tree from a ROOT file and constructs neutrino numpy array
    """
    ...
    neutrino_ids = truth["W_decay_id"].array()[truth_leptonic_mask*up_type_mask]
    ...
    neutrino_data = np.zeros((len(neutrino_ids), N_neutrinos), dtype=neutrino_dt)
...
def parse_MET(reco):
    ...
    met_data = np.zeros((len(reco["met_met"].array())), dtype=met_dt)
    met_data["MET"] = reco["met_met"].array().to_numpy()[:,0]
    met_data["phi"] = reco["met_phi"].array().to_numpy()[:,0]
    
    return met_data
    
def parse_event_level(reco):
    
    """
    This assumes that the truth and reco trees are already matched
    """
    # Event-level data
    njets  = ak.num(reco["jet_btag"].array())
    nbjets = ak.count_nonzero(reco["jet_btag"].array(),axis=1)
    eventNumber = ak.Array(np.arange(len(reco["jet_btag"].array())))
...
def main(infile_name,outfile_name,split_ratio):
    ...
    njets , nbjets, eventNumber = parse_event_level(reco)
    
    print("Parsing jet data")
    jet_data      = parse_jets(reco)
    
    print("Parsing lepton data")
    lepton_data   = parse_leptons(reco)
    
    print("Parsing neutrino data")
    neutrino_data = parse_neutrinos(truth)
    
    print("Parsing MET data")
    met_data      = parse_MET(reco)
    ...
    outfile_name = outfile_name.replace(".h5","")
